...
const InspectionTab = ({ staff, farmers, droneReports, blockchainLog, onInspect }) => {
  ...
  const startInspection = (farmer) => {
    ...
    const aggregated = aggregateAnalyses(reports);
    if (aggregated) {
          ...
          {farmers.map(f => {
            ...
            return (
              <div key={f.id} className="border border-slate-200 rounded-xl p-3 hover:border-emerald-300 transition-all">
                {/* Check if already inspected in this session */}
                {(() => {
                  const isInspected = blockchainLog.some(l => l.action === "FIELD_INSPECTION" && l.data.includes(f.hoTen));
                  return (
                    <>
        ...
        <div className="bg-white rounded-2xl border-2 border-emerald-300 shadow-xl p-6">
          <div className="flex items-start justify-between mb-4 flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Drone đã bay trước. Xem các vùng có vấn đề → xuống đồng chụp ảnh thực địa bổ sung → AI tổng hợp.</p>
            </div>
              ...
              <>
                <div className="bg-purple-50 border-2 border-purple-200 rounded-xl p-3 mb-4">
                  <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                    <div className="text-xs font-bold text-purple-900">📊 Tổng hợp {agg.count} ảnh (AND nghiêm cho SRP)</div>
                  ...
                  {agg.diseases.length > 0 && (
                    <div className="bg-white rounded-lg p-2 border border-rose-200 mb-2 mt-2">
                      <div className="text-[11px] font-bold text-rose-800 uppercase mb-1">🦠 Sâu bệnh từ ảnh ({agg.diseases.length})</div>
                  ...
                  {agg.recommendations.length > 0 && (
                    <div className="bg-white rounded-lg p-2 border border-amber-200 mt-2">
                ...
                {farmerDroneReports.length === 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 text-xs text-amber-800">
                    ⚠️ <b>Chưa có báo cáo drone</b> cho ruộng này. Bạn có thể up ảnh thực địa để AI đánh giá.
                  </div>
                )}
                  ...
                  {all.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {farmerDroneReports.map((r, i) => (
                        <PhotoCard key={`d-${i}`} analysis={r} type="drone" index={i} />
                      ))}
                      {fieldAnalyses.map((a, i) => (
                        <PhotoCard key={`f-${i}`} analysis={a} type="field" index={i} />
                      ))}
                    </div>
                  )}
                </div>
          ...
          <div onClick={goUploadField} className="w-full text-center py-3 mb-6 rounded-xl border-2 border-emerald-600 text-emerald-700 font-bold text-sm cursor-pointer hover:bg-emerald-50 shadow-sm select-none">
            📷 Up thêm ảnh thực địa (chọn nhiều) → AI tổng hợp
          </div>

          <div className="flex items-center justify-between mt-6 mb-4 pt-6 border-t border-slate-200">
            <div>
...
const PhotoCard = ({ analysis: r, type, index }) => (
  <div className={`border ${type === "drone" ? "border-sky-200 bg-sky-50/30" : "border-emerald-200 bg-emerald-50/30"} rounded-xl overflow-hidden`}>
    <img src={r.previewUrl} alt="" className="w-full h-28 object-cover" />
    <div className="p-2.5 space-y-1.5">
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className={`text-[10px] ${type === "drone" ? "bg-sky-100 text-sky-800" : "bg-emerald-100 text-emerald-800"} px-1.5 py-0.5 rounded font-bold`}>
          {type === "drone" ? `🚁 Drone #${index + 1}` : `📷 Thực địa #${index + 1}`}
        </span>
