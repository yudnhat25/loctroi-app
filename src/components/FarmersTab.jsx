    ...
    <div className="space-y-8 fade-in pb-10">
      ...
      <section className="bg-white rounded-2xl border border-surface-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-200 flex items-baseline justify-between">
          <h3 className="text-[17px] font-display font-semibold text-slate-900 tracking-tight">Phân bố Tier nông dân</h3>
          <span className="text-[12px] text-slate-500">{farmers.length} hộ liên kết</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-surface-200">
          {["A", "B", "C", "D"].map(code => {
            ...
            return (
              <div key={code} className="bg-white px-5 py-4">
                <div className="flex items-center gap-2">
                  <span className={`w-1.5 h-1.5 rounded-full ${accents.dot}`}></span>
                  <span className="text-[12px] font-semibold uppercase tracking-[0.14em] text-slate-500">Tier {code}</span>
                </div>
                <div className={`font-display text-[34px] font-semibold tabular leading-none mt-3 ${accents.ink}`}>
                  {tierCounts[code] || 0}
                </div>
                <p className="text-[12px] text-slate-500 mt-2">{accents.label}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="bg-white rounded-2xl border border-surface-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-200">
          <div>
            <h2 className="text-[17px] font-display font-semibold text-slate-900 tracking-tight">Danh sách hộ liên kết</h2>
            <p className="text-[12px] text-slate-500 mt-0.5">Hộ chiếu số trên blockchain Hyperledger</p>
          </div>
          <span className="text-[12px] font-mono text-slate-400">{farmers.length} entries</span>
        </div>
      ...
      <section className="bg-white rounded-2xl border border-surface-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-200">
          <h2 className="text-[17px] font-display font-semibold text-slate-900 tracking-tight">Danh mục vật tư</h2>
          <p className="text-[12px] text-slate-500 mt-0.5">{supplies.length} loại đang phân phối</p>
        </div>
        <ul className="divide-y divide-surface-200">
          {supplies.map(s => (
            <li key={s.id} className="px-6 py-3.5 flex items-center justify-between gap-4 hover:bg-surface-50/60 transition-colors">
              <div className="min-w-0">
                <p className="text-[14px] font-semibold text-slate-900">{s.ten}</p>
                <p className="text-[12px] text-slate-500 mt-0.5">Đơn vị: {s.donVi}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="font-display text-[18px] font-semibold tabular text-brand-700">
                  {formatVND(s.donGia)}<span className="text-[12px] text-slate-400 font-normal ml-1">/{s.donVi}</span>
                </p>
              </div>
            </li>
          ))}
      ...
      <section className="bg-white rounded-2xl border border-surface-200 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-surface-200">
          <div>
            <h2 className="text-[17px] font-display font-semibold text-slate-900 tracking-tight">Yêu cầu vật tư chờ duyệt</h2>
            <p className="text-[12px] text-slate-500 mt-0.5">Smart contract đối chiếu Tier, bạn xác nhận hoặc từ chối</p>
          </div>
        ...
        {supplyRequests?.length === 0 ? (
          ...
          <ul className="divide-y divide-surface-200">
            {supplyRequests.map(req => {
              ...
              return (
                <li key={req.id} className="px-6 py-4 hover:bg-surface-50/60">
                  <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="font-mono font-semibold text-slate-700 text-[14px]">{req.id}</span>
                        ...
                        <span className="text-[12px] text-slate-400">{req.season}</span>
                      </div>
                      <div className="font-semibold text-slate-900 text-[15px]">{req.farmer.hoTen}</div>
                      <div className="text-[12px] text-slate-500">{req.farmer.htx ?? req.farmer.diaChi} · {req.farmer.dienTich} ha · {req.farmer.digitalId ?? req.farmer.id}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">Tổng giá trị</div>
                      <div className="font-display text-[20px] font-semibold tabular text-amber-700 mt-1 leading-none">{formatVND(req.total ?? 0)}</div>
                    </div>
                  </div>

                  <div className="bg-surface-50 rounded-lg ring-1 ring-surface-200 px-3 py-2 mb-3">
                    <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 mb-1.5">{items.length} loại vật tư</div>
                    <div className="flex flex-wrap gap-1.5">
                      {items.map((it, i) => (
                        <span key={i} className="inline-flex items-center gap-1 bg-white ring-1 ring-surface-200 rounded-md px-2 py-0.5 text-[12px] text-slate-700">
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center justify-center border border-rose-200 text-rose-700 hover:bg-rose-50 px-4 py-2 rounded-lg text-[14px] font-semibold transition-colors"
                    >
                      Từ chối
                    </button>
                    <button
                      onClick={() => onApproveRequest(req)}
                      className="inline-flex items-center justify-center bg-brand-700 hover:bg-brand-800 text-white px-4 py-2 rounded-lg text-[14px] font-semibold transition-colors"
                    >
                      Duyệt và chuyển tài xế
                    </button>
                  </div>
                </li>
              );
      ...
      <section className="bg-white rounded-2xl border border-surface-200 px-6 py-5">
        <div className="flex items-baseline justify-between mb-5">
          <h3 className="text-[17px] font-display font-semibold text-slate-900 tracking-tight">Sổ cái blockchain</h3>
          <span className="text-[12px] font-mono text-slate-400">{blockchainLog.length} blocks</span>
        </div>
        <ol className="relative space-y-3.5 before:absolute before:left-[7px] before:top-1 before:bottom-1 before:w-px before:bg-surface-200">
          {blockchainLog.map((log, idx) => (
            <li key={idx} className="relative pl-6">
              ...
              <div className="bg-surface-50/70 rounded-lg ring-1 ring-surface-200 px-3.5 py-2.5">
                ...
                <p className="text-[14px] text-slate-700 leading-relaxed">{log.data}</p>
              </div>
            </li>
          ))}
        </ol>
