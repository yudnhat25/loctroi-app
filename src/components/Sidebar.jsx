...
export const TAB_DEF = {
  ...
  buyerSales: { label: "HĐ Buyer & Factoring", icon: ContainerIcon },
  invoices: { label: "Khoản phải thu nội bộ", icon: FileTextIcon },
  officerHome: { label: "Trang chủ Đồng ruộng", icon: LayoutDashboardIcon },
...
const Sidebar = ({ activeTab, setActiveTab, blockchainLog, invoices, buyerInvoices = [], farmerApplications = [], role, subrole, profile, onLogout, isSidebarOpen, setIsSidebarOpen }) => {
  const pendingScf = invoices.filter(i => i.trangThai === "Chào bán ngân hàng").length
    + buyerInvoices.filter(i => i.trangThai === "Đã token hóa" || i.trangThai === "Chào bán ngân hàng").length;
          ...
          {availableTabs.map(id => {
            ...
            return (
              <button
                ...
                className={`group w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-300 text-[14px] font-medium ${isActive
