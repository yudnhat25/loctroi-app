          ...
          {progress.map((step, i) => {
            ...
            return (
              <li key={step.key} className="relative pl-12 flex items-center justify-between gap-4 min-h-[40px]">
                {/* Step dot */}
                <span
                  className={`absolute left-0 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center ring-2 ring-white ${step.done
...
const QuickAction = ({ icon, label, sub, onClick, accent, disabled, active }) => (
  <button
    ...
    disabled={disabled}
    className={`group relative bg-white rounded-2xl border ${active ? "border-brand-600 ring-1 ring-brand-200" : "border-surface-200"} px-2 sm:px-3 py-3 sm:py-4 flex flex-col items-center text-center transition-colors ${disabled ? "opacity-50 cursor-not-allowed" : "hover:border-surface-300 cursor-pointer"
      }`}
  >
